...
│                                                                                                  │
│  ┌────────────────────┐  ┌────────────────────┐  ┌────────────────────┐  ┌────────────────────┐ │
│  │  Market Service    │  │  Market Service(内嵌策略引擎)  │  │  Execution Service │  │   Order Service    │ │
│  │ (market.demo/      │  │ (market.demo/      │  │  (execution.yaml)  │  │  (order.yaml)      │ │
...
│  │  • signal               → Market Service(内嵌策略引擎) 发布交易信号                                   │    │
│  │  • order                → Execution Service 发布订单状态事件                              │    │
│  │  • harvest_path_signal  → Market Service(内嵌策略引擎) 发布收割路径风险信号                           │    │
│  └──────────────────────────────────────────────────────────────────────────────────────────┘    │
│                                                                                                  │
└──────────────────────────────────────────────────────────────────────────────────────────────────┘
            │                      │                      │                      │
...
sequenceDiagram
    ...
    participant STR as Market Service(内嵌策略引擎)
...
2. Strategy Universe: 再决定“启哪个策略模板”
   • 所在位置: strategy
   • 核心输入: market 内嵌策略侧最近一根 1m 快照 + marketstate
...
4. Regime Judge
   ...
   • 另一部分在 market 内嵌策略侧 marketstate 检测
   • 功能已存在，但还没有完全收敛成单一判态层

5. Strategy Router
   • 当前最接近 market 内嵌策略侧 universe.Selector + applyUniverseDecision
...
7. Risk Manager / Execution Engine
   • 当前已经比较清晰
   • market 内嵌策略侧负责部分信号风控，execution 侧负责更硬的执行风控与下单
```
...
• market 内嵌策略侧的 marketstate 特征构建
...
| 2. 判断每个币 Market State | `marketstate.Result` | `data/signal/marketstate/{SYMBOL}/{YYYY-MM-DD}.jsonl`、`data/signal/marketstate/_meta/{YYYY-MM-DD}.jsonl` | 全局状态聚合、Universe 模板切换、weights 输入 | `market/rpc/internal/marketstate/` |
...
| 4. 分配策略权重 | `weights.Output`、`weights.Recommendation` 中的 `strategy_weight` / `risk_scale` / `market_paused` | `data/signal/weights/_meta/{YYYY-MM-DD}.jsonl` | Symbol 权重计算、发单前风险缩放、暂停交易判定 | `market/rpc/internal/weights/engine.go` |
| 5. 选 Top N 币 | 当前仍以 `[]universe.DesiredStrategy` 和候选 symbol 集为主，尚未形成独立 `TopN` 结构 | `data/signal/universe/{SYMBOL}/{YYYY-MM-DD}.jsonl`、`data/signal/universe/_meta/{YYYY-MM-DD}.jsonl` | 仓位分配、策略启停、模板切换 | `market/rpc/internal/universe/selector.go` |
...
• 说明：
  ...
  market 内嵌策略侧会在 1m K 线进入后构建 snapshot，
  当前已稳定使用 EMA / RSI / ATR / Volume。

Step 2. 判断每个币 Market State
• 当前状态：已完成基础版
• 说明：
  market 内嵌策略侧已能对每个 symbol 输出 trend_up / trend_down / range / breakout / unknown。
...
| [strategy](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/strategy) | 实现基础策略判断、信号生成与多周期执行逻辑 |
| [decision logs](file:///Users/bytedance/GolandProjects/exchange-system/data/signal/decision) | 落地策略决策日志，回答“为什么这轮开或不开” |
| [signal logs](file:///Users/bytedance/GolandProjects/exchange-system/data/signal) | 落地最终信号日志，回答“这轮到底发了什么 signal” |
...
| [features.go](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/marketstate/features.go) | 把上游价格特征接入 `strategy` 侧市场状态链路 |
| [serviceContext.go](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/svc/serviceContext.go) | 在 `strategy` 侧把 snapshot、state、weights 串成一条运行链 |
...
| [marketstate](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/marketstate) | 在 `strategy` 侧做单币状态识别和全局状态聚合 |
| [universe](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/universe) | 负责 `strategy universe` 的启停、模板切换与最小策略路由 |
| [weights](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/weights) | 负责策略桶权重、币种权重、风险缩放和暂停交易逻辑 |
| [trend_following.go](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/strategy/trend_following.go) | 在策略实例里真正应用权重结果，把预算折算到开仓数量 |
| [universepool](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/universepool) + [universe](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/universe) + [marketstate](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/marketstate) + [weights](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/weights) + [serviceContext.go](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/svc/serviceContext.go) | 共同拼出当前“统一动态决策引擎”的最小主链路 |
...
| [breakout.go](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/strategy/breakout.go) | 已有 `Breakout` 逻辑雏形，但仍作为 `trend_following` 变体运行；对应策略说明见 [breakout_strategy_technical_doc.md](file:///Users/bytedance/GolandProjects/exchange-system/docs/breakout_strategy_technical_doc.md) |
| [range.go](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/strategy/range.go) | 已有 `Range/MeanReversion` 逻辑雏形，但仍作为 `trend_following` 变体运行；对应策略说明见 [volatility_strategy_technical_doc.md](file:///Users/bytedance/GolandProjects/exchange-system/docs/volatility_strategy_technical_doc.md) |
...
| 模块 | 作用 |
|------|------|
| [weights](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/weights) | 当前最接近 `统一 Position Allocator` 的位置，但还不是独立完整模块 |
...
| [harvestpath](file:///Users/bytedance/GolandProjects/exchange-system/app/market/rpc/internal/strategy/harvestpath) | 已有订单簿风险过滤与路径判断能力，但还停留在辅助层，不是主决策核心 |
...
• app/market/rpc/internal/marketstate/detector.go
  已开始复用统一 Analysis，再输出 market 内嵌策略侧 MarketState Result
...
```text
• app/market/rpc/internal/strategyrouter/router.go
  已经是第一版显式 Router

• app/market/rpc/internal/universe/selector.go
...
```text
• app/market/rpc/internal/weights/engine.go
  已是第一版 Position Allocator 主体

• app/market/rpc/internal/weights/logger.go
...
• app/market/rpc/internal/strategyrouter
• app/market/rpc/internal/weights
• app/market/rpc/internal/svc/serviceContext.go
```
...
3. Router 已独立，但还只是 market 内嵌策略侧路由中心，不是更广义决策中心
...
| **Market Service** | market.demo.yaml / market.prod.yaml | gRPC | 行情数据聚合、技术指标计算、WebSocket客户端，并内嵌策略引擎 |
...
| 策略执行信号 | Market Service(内嵌策略引擎) | Execution Service | Kafka | signal |
...
2. market 内嵌策略配置
   ...
   -> StrategyEngine.Strategies 中增加对应 symbol 的策略实例
...
4. 再确认 market 内嵌策略
   看 app/market/rpc/data/kline/{symbol}/...
...
# 看 market 内嵌策略是否拿到对应 symbol 的 1m 输入
sed -n '1,20p' app/market/rpc/data/kline/BTCUSDT/1m/$(date -u +%F).jsonl

# 看 decision / signal
sed -n '1,20p' data/signal/decision/BTCUSDT/$(date -u +%F).jsonl
sed -n '1,20p' data/signal/BTCUSDT/$(date -u +%F).jsonl
```
...
```text
• 只改了 gateway，但没把 `Strategy.Target` 指到 `market:9080`
• 只改了 `market.demo.yaml`，真实运行却用了 `market.prod.yaml`
• market 有 15m，但内嵌策略因非 final 不落盘，被误判为“没消费到”
...
```text
data/signal/universe/_meta/{date}.jsonl
```
...
```text
路径:
data/signal/universe/{symbol}/{date}.jsonl
...
# 看 Universe 总览
sed -n '1,20p' data/signal/universe/_meta/$(date -u +%F).jsonl

# 看某个 symbol 的明细
sed -n '1,20p' data/signal/universe/BTCUSDT/$(date -u +%F).jsonl

# 只看模板切换相关字段
grep '"base_template"\|"template"\|"action"\|"reason"' \
  data/signal/universe/BTCUSDT/$(date -u +%F).jsonl
```
...
• 不包含 market 内嵌策略侧的 universe 调度和 signal 生成
...
1. strategy 生成开仓信号
   ...
   文件: data/signal/SOLUSDT/2026-05-01.jsonl
   关键信息:
   • action=BUY
   • side=LONG
...
3. 日志侧：
   建议新增:
   data/signal/marketstate/{symbol}/{date}.jsonl
```
...
```text
1. data/signal/marketstate/_meta/{date}.jsonl
2. data/signal/marketstate/{symbol}/{date}.jsonl
```
...
# 看 MarketState 总览
sed -n '1,20p' data/signal/marketstate/_meta/$(date -u +%F).jsonl

# 看 BTC 的状态识别明细
sed -n '1,20p' data/signal/marketstate/BTCUSDT/$(date -u +%F).jsonl
...
```text
data/signal/weights/_meta/{date}.jsonl
data/signal/weights/{symbol}/{date}.jsonl
```
...
```text
1. data/signal/weights/_meta/{date}.jsonl
2. data/signal/weights/{symbol}/{date}.jsonl
3. strategy 终端日志中的 [weights] evaluate ...
4. data/signal/{symbol}/{date}.jsonl 中的 weights 字段
```
...
# 看 Weights 总览
sed -n '1,20p' data/signal/weights/_meta/$(date -u +%F).jsonl

# 看 BTC 的权重建议
sed -n '1,20p' data/signal/weights/BTCUSDT/$(date -u +%F).jsonl
...
grep '"route_' data/signal/BTCUSDT/$(date -u +%F).jsonl | tail -n 5
```
...
|-------|--------|--------|---------|------|
| kline | Market Service | Market Service(内嵌策略引擎) | Kline JSON | 聚合K线数据 |
| signal | Market Service(内嵌策略引擎) | Execution Service | Signal JSON | 交易信号 |
...
#### 6. market 有 15m，但内嵌策略还没形成对应信号

| 项 | 内容 |
|----|------|
| 现象 | `app/market/rpc/data/kline/.../15m/...` 已产出，但 `data/signal/decision/...` 还没有对应判定 |
| 第一眼看 | 这根 `15m` 的 `isFinal` 和 `isDirty` |
| 第二眼看 | `app/market/rpc/internal/aggregator/emit.go` 和策略日志落盘规则 |
| 最可能根因 | `market` 内嵌策略引擎依赖闭合高周期，非 final 的 `15m` 即使已聚合也不会形成最终判定 |

#### 7. market 内嵌策略为什么没发信号

| 项 | 内容 |
|----|------|
| 现象 | 有 `kline`，但没有 `signal` |
| 第一眼看 | `data/signal/decision/ETHUSDT/*.jsonl` |
...
| 第二眼看 | `app/market/rpc/data/kline/ETHUSDT/1h/*.jsonl` 和 `4h/*.jsonl` |
...
| 第二眼看 | `app/market/rpc/data/kline/ETHUSDT/4h/*.jsonl` |
...
#### 2. 看 market / execution / order 启动日志
...
# market 内嵌策略侧 1h
sed -n '1,20p' app/market/rpc/data/kline/ETHUSDT/1h/2026-04-25.jsonl

# market 内嵌策略侧 4h
sed -n '1,20p' app/market/rpc/data/kline/ETHUSDT/4h/2026-04-25.jsonl
```
...
# 找 market 内嵌策略侧同一根 1h
grep '2026-04-25T02:00:00.000Z' app/market/rpc/data/kline/ETHUSDT/1h/2026-04-25.jsonl
```
...
```bash
# 看今天 decision 前 20 条
sed -n '1,20p' data/signal/decision/ETHUSDT/2026-04-25.jsonl

# 看今天 signal 前 20 条
sed -n '1,20p' data/signal/ETHUSDT/2026-04-25.jsonl

# grep 某个 reason_code
grep '"reason_code":"h4_no_trend"' \
  data/signal/decision/ETHUSDT/2026-04-25.jsonl
...
# market 内嵌策略侧找 worker_gc
grep '"dirtyReason":"worker_gc' \
  app/market/rpc/data/kline/ETHUSDT/1h/2026-04-25.jsonl
```
...
# decision
grep '2026-04-25 12:15:59' data/signal/decision/ETHUSDT/2026-04-25.jsonl
```
...
5. 回到 `app/market/rpc/data/kline/ETHUSDT/4h/*.jsonl` 看对应 `4h` 记录
...
# 看 market 内嵌策略配置中的 Harvest Path 开关
grep -n 'HarvestPath' app/market/rpc/etc/market.demo.yaml
```
...
```text
当交易对从 1 个扩展到 5 个以上时，
market 配置里的 StrategyEngine.Templates 中会出现多份几乎完全相同的参数块，
...
1. 在 `app/market/rpc/internal/strategy/` 创建策略文件
