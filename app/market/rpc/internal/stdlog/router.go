...
func newRouter(baseDir string) (*Router, error) {
	...
	router := &Router{
		...
		rules: []RouteRule{
			{name: "market.recovery.log", patterns: []string{"[market recovery]"}},
			{name: "market.websocket.log", patterns: []string{"[ws", "WebSocket", "Connecting to Binance WebSocket", "Binance symbols updated", "Using SOCKS5 proxy", "Using HTTP proxy", "Invalid proxy URL", "Invalid SOCKS5 proxy"}},
			{name: "market.aggregator.log", patterns: []string{"[aggregator]", "[aggregated", "[agg-log]", "[warmup]"}},
...
func (r *Router) rotateFilesIfNeeded(now time.Time) error {
	...
	allFile, err := openLogFile(r.baseDir, dateDir, "market.all.log")
	if err != nil {
		return err
	}
	defaultFile, err := openLogFile(r.baseDir, dateDir, "market.misc.log")
	if err != nil {
		_ = allFile.Close()
		return err
	}
