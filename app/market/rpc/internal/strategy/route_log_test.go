...
// TestWriteDecisionLogSummaryIncludesH1RejectReasons 验证 h1_no_pullback 会自动补齐失败条件并写入顶层 reason。
func TestWriteDecisionLogSummaryIncludesH1RejectReasons(t *testing.T) {
	dir := t.TempDir()
	...
	s.writeDecisionLogIfEnabled("entry", "skip", "h1_no_pullback", k, map[string]interface{}{
		"trend":              "LONG",
		"h1_open_time":       "2026-05-05 08:00:00 UTC",
		"h1_is_dirty":        false,
		"h1_is_tradable":     true,
		"h1_is_final":        true,
		...
		"structure_ok":       true,
		"ema_trend_ok":       true,
		"crossover_cross_ok": false,
		"crossover_price_ok": false,
		"crossover_adx_ok":   false,
		"crossover_macd_ok":  false,
	})

	entry := readSingleJSONLine(t, filepath.Join(dir, "decision", "ETHUSDT", now.Format("2006-01-02")+".jsonl"))
	extras, ok := entry["extras"].(map[string]interface{})
	if !ok {
		t.Fatalf("extras = %#v, want object", entry["extras"])
	}
	descs, ok := extras["reject_descs"].([]interface{})
	if !ok || len(descs) == 0 {
		t.Fatalf("reject_descs = %#v, want generated H1 reject descriptions", extras["reject_descs"])
	}
	if got, _ := entry["reason"].(string); got == "" || got == "1小时没有满足回调或交叉入场条件" {
		t.Fatalf("reason = %#v, want detailed reject text", entry["reason"])
	}
	if got := extras["summary"]; got == nil || got == "" {
		t.Fatalf("extras.summary = %#v, want reject summary", got)
	}
}
