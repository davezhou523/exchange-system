...
func TestEvaluateEnterLong(t *testing.T) {
	...
	decision, err := engine.Evaluate(Snapshot{
		Symbol:    "BTCUSDT",
		H4:        h4,
		H1:        h1,
		M15:       m15,
		Timestamp: m15[len(m15)-1].CloseTime,
	}, State{}, Account{Equity: 10000, AvailableCash: 10000})
	if err != nil {
...
func TestRiskPauseAfterConsecutiveLosses(t *testing.T) {
	engine := NewEngine(DefaultParams())
	now := time.Date(2026, 4, 5, 12, 0, 0, 0, time.UTC)
	h4 := candlesFromCloses(now.Add(-4*time.Hour*59), 4*time.Hour, risingSeries(100, 59, 1))
	h1 := candlesFromCloses(now.Add(-time.Hour*64), time.Hour, risingSeries(100, 65, 1))
	...
	}, Account{Equity: 10000, AvailableCash: 10000})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if decision.Reason != "达到最大连续亏损限制" {
		t.Fatalf("unexpected reason: %s", decision.Reason)
	}
}

func candlesFromCloses(start time.Time, step time.Duration, closes []float64) []Candle {
